...
export function BarcodeInput({ onScan, isLoading }: BarcodeInputProps) {
  ...
  const startCamera = async () => {
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Failed to access camera. Please check permissions."
          ...
          >
            <Camera
              className={`w-3 h-3 sm:w-4 sm:h-4 ${
                cameraActive ? "text-green-500" : cameraLoading ? "text-yellow-500 animate-spin" : ""
              }`}
            />
          </button>
        </div>
      ...
      {showCamera && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          <div className="bg-black/80 backdrop-blur-sm p-3 sm:p-4 flex items-center justify-between z-10">
            <h3 className="text-white font-semibold text-base sm:text-lg">Scan Barcode</h3>
            <button
              onClick={stopCamera}
          ...
          <div className="flex-1 relative overflow-hidden bg-black">
            ...
            <div className="absolute inset-0">
              ...
              <div
                ...
                style={{
                  background: "radial-gradient(ellipse 400px 350px at center, transparent 0%, rgba(0,0,0,0.85) 70%)",
                }}
              />

              <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="relative w-full max-w-sm" style={{ aspectRatio: "5/4" }}>
                  {/* Bright green glowing border */}
                  <div
                    className="absolute inset-0 rounded-2xl sm:rounded-3xl"
                    style={{
                      border: "4px solid rgb(34, 197, 94)",
                      boxShadow: "0 0 30px rgba(34, 197, 94, 0.8), inset 0 0 20px rgba(34, 197, 94, 0.2)",
                    }}
                  />
            ...
            >
              <div className="flex flex-col items-center gap-2 sm:gap-3 max-w-md mx-auto">
                ...
                <p className="text-white text-lg sm:text-xl md:text-2xl font-bold drop-shadow-lg">
                  {cameraLoading ? "Loading camera..." : "Place Barcode in Green Frame"}
                </p>
                <p className="text-gray-200 text-sm sm:text-base md:text-lg drop-shadow-md px-4">
                  Position your barcode inside the bright green rectangle and hold steady
                </p>
              </div>
            </div>
          </div>
