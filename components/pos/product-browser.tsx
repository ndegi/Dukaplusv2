...
export function ProductBrowser({ onAddToCart, searchTerm = "" }: ProductBrowserProps) {
  ...
  if (isLoading) {
    return <div className="card-base p-8 text-center text-secondary">Loading products...</div>
  }

  if (error) {
    return (
      <div className="alert-error flex items-start gap-3">
        <FontAwesomeIcon icon={faExclamationCircle} className="text-danger mt-0.5" />
    ...
    <div className="space-y-4 p-4 sm:p-6 bg-background">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
        {filteredProducts.length === 0 ? (
          <div className="col-span-full text-center py-8 text-secondary">No products found</div>
        ) : (
          filteredProducts.map((product) => (
            <div
              key={product.id}
              className="card-hover overflow-hidden cursor-pointer flex flex-col justify-between"
            ...
            >
              ...
              <div className="p-3 flex-1 flex flex-col justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-foreground text-xs line-clamp-2">{product.name}</h3>
                  <p className="text-xs text-foreground mt-0.5 font-medium">
                    <FontAwesomeIcon icon={faBox} /> {product.quantity} available
                  </p>
                </div>
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="btn-success w-8 h-8 rounded flex items-center justify-center flex-shrink-0 ml-2"
                  >
