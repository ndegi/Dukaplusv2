...
export function PurchaseInvoicesManager() {
  ...
  const handleEditInvoice = async (invoice: PurchaseInvoice) => {
    try {
      const response = await fetch(`/api/purchase-invoices?invoice_id=${invoice.name}`)
      const data = await response.json()
  ...
  const handleSaveEdit = async () => {
    ...
    setConfirmDialog({
      ...
      action: async () => {
        ...
        try {
          const orderId = (editingInvoice as any).order_id || (editingInvoice as any).purchase_order
