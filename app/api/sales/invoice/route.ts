...
export async function POST(request: NextRequest) {
  try {
    ...
    // Include draft_id if completing a draft invoice
    if (body.draft_id) {
      payload.sales_id = body.draft_id;
    }

    const response = await fetch(
      `${credentials.baseUrl}/api/method/dukaplus.services.rest.create_sales_invoice`,
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      return NextResponse.json({
        ...
        data: data.message,
        sales_id: data.message?.sales_id,
      });
    } else {
      return NextResponse.json(
        {
