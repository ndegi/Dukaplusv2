...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(
      `${creds.baseUrl}/api/method/dukaplus.services.rest.get_stock_balance_report`
    );
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.message || "Failed to fetch stock balance" },
    ...
    return NextResponse.json(
      { error: "Failed to fetch stock balance" },
      { status: 500 }
    );
  }
}
