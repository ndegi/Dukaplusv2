...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${creds.baseUrl}/api/method/dukaplus.services.rest.sales_performance_over_time`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ message: "Failed to fetch sales performance" }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json(data.message || {})
  } catch (error) {
    console.error("[DukaPlus] Sales performance fetch error:", error)
