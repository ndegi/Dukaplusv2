...
export async function POST(request: NextRequest) {
  try {
    ...
    const { warehouse = "", sales_person = "", from_date = "", to_date = "" } = body

    const creds = JSON.parse(credentials)

    const response = await fetch(`${creds.baseUrl}/api/method/dukaplus.services.rest.productivity_per_product`, {
      method: "POST",
      headers: {
